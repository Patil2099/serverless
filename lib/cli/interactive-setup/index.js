'use strict';

const inquirer = require('@serverless/utils/inquirer');

const steps = {
  service: require('./service'),
  dashboardLogin: require('@serverless/dashboard-plugin/lib/cli/interactive-setup/dashboard-login'),
  dashboardSetOrg: require('@serverless/dashboard-plugin/lib/cli/interactive-setup/dashboard-set-org'),
  awsCredentials: require('./aws-credentials'),
  autoUpdate: require('./auto-update'),
  deploy: require('./deploy'),
};

module.exports = async (context) => {
  context = { ...context, inquirer, history: new Map(), stepDurations: new Map() };
  const stepDurations = new Map();
  for (const [stepName, step] of Object.entries(steps)) {
    delete context.stepHistory;
    const stepData = await step.isApplicable(context);
    const stepStartTime = process.hrtime();
    if (stepData) {
      process.stdout.write('\n');
      context.stepHistory = [];
      context.history.set(stepName, context.stepHistory);
      await step.run(context, stepData);
      const stepDuration = process.hrtime(stepStartTime);
      const stepDurationMs = Math.floor(stepDuration[0] * 1000 + stepDuration[1] / 1000000);
      stepDurations.set(stepName, stepDurationMs);
    }
  }
  return { history: context.history, stepDurations };
};
