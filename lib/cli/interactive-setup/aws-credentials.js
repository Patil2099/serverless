...
const awsAccessKeyIdInput = async ({ stepHistory }) => {
  ...
  stepHistory.push(['accessKeyId', '_user_provided_']);
  return accessKeyId;
};
...
const awsSecretAccessKeyInput = async ({ stepHistory }) => {
  ...
  stepHistory.push(['secretAccessKey', '_user_provided_']);
...
const steps = {
  ...
  ensureAwsAccount: async ({ stepHistory }) => {
    ...
    stepHistory.push(['createAwsAccountPrompt', true]);
  },
  ensureAwsCredentials: async ({ options, configuration, stepHistory }) => {
    ...
    stepHistory.push(['generateAwsCredsPrompt', true]);
  },
  ...
  handleProviderCreation: async ({ configuration: { org: orgName }, stepHistory }) => {
    ...
    const p = new Promise((resolve) => {
      ...
      showSkipPromptTimeout = setTimeout(() => {
        ...
        inquirerPrompt.then(() => {
          stepHistory.push(['skipProviderSetup', true]);
          resolve(null);
        });
      }, timeoutDuration);
