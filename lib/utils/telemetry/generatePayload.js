...
module.exports = async ({
  ...
  if (commandUsage) {
    payload.commandDataUsage = { durations: {}, steps: [] };
    for (const [step, duration] of commandUsage.stepDurations) {
      payload.commandDataUsage.durations[step] = duration;
    }

    for (const [step, stepHistory] of commandUsage.history) {
      payload.commandDataUsage.steps.push([step, stepHistory]);
    }
  }
